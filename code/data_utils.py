...
def load_sun_df():
    sun_df = pd.read_csv("../data/sun.csv")
    sun_df['sun_date'] = pd.to_datetime(sun_df.sun_date, format='%Y-%m-%d')
    sun_df['sun_rise'] = sun_df.sun_rise.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))+sun_df.sun_date
    sun_df['sun_mid'] = sun_df.sun_mid.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))+sun_df.sun_date
    sun_df['sun_set'] = sun_df.sun_set.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))+sun_df.sun_date
    sun_df['day_length'] = sun_df.day_length.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))
    sun_df['day_in'] = sun_df.day_in.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))+sun_df.sun_date
    sun_df['day_out'] = sun_df.day_out.apply(lambda x: pd.Timedelta(x, format='%H:%M:%S'))+sun_df.sun_date
    return sun_df
