...
def get_obs(df):
    obs_cols = [obs_col for obs_col in df.columns if '_obs' in obs_col]
    df_obs = df[['stations', 'dates'] + obs_cols].drop_duplicates()
    return df_obs.sort_values(['stations', 'dates']).reset_index(drop=True)
...
df_obs['dates'] += pd.Timedelta('17 hours')
df_obs = df_obs[
    ~df_obs[obs_cols].isnull().all(1)
]
df_obs = df_obs[df_obs.dates >= pd.to_datetime('2015-03-02')]

arr_nday = pd.DataFrame([
    df_obs.dates.dt.date.unique(), 
    list(range(df.nday.max(), 1, -1))[::-1]
]).T.set_index(0).loc[df_obs.dates.dt.date, 1].values
df_obs.insert(1, 'nday', arr_nday)
df_obs.drop('dates', axis=1, inplace=True)

df_last24h = df_obs.groupby('nday')[obs_cols].mean().reset_index().drop('d10m_obs', axis=1)
df_last24h = df_last24h.rename(columns=dict(zip(df_last24h.columns[1:], df_last24h.columns[1:] + '_last24h')))
df_last24h_stations_mean = df_obs.groupby(['nday', 'stations'])[obs_cols].mean().reset_index().drop('d10m_obs', axis=1)
df_last24h_stations_mean = df_last24h_stations_mean.rename(columns=dict(
    zip(df_last24h_stations_mean.columns[2:], df_last24h_stations_mean.columns[2:] + '_last24h_stations_mean')))
df_last24h_stations_std = df_obs.groupby(['nday', 'stations'])[obs_cols].std().reset_index().drop('d10m_obs', axis=1)
df_last24h_stations_std = df_last24h_stations_std.rename(columns=dict(
    zip(df_last24h_stations_std.columns[2:], df_last24h_stations_std.columns[2:] + '_last24h_stations_std')))
...
df_all = df_all.merge(df_last24h, left_on='nday', right_on='nday', how='left')\
               .merge(df_last24h_stations_mean, left_on=['nday', 'stations'], right_on=['nday', 'stations'], how='left')\
               .merge(df_last24h_stations_std, left_on=['nday', 'stations'], right_on=['nday', 'stations'], how='left')
...
val_days = 30
df_val = df_train[lambda x: (x.nday >= split_end-val_days+1) & (x.nday <= split_end) & (x.foretimes >= 4)]
df_tr = df_train[lambda x: (x.nday < split_end-val_days+1) & (x.foretimes >= 4)]
...
bst_t2m.save_model('bst_t2m_gbdt_{}_new2.txt'.format(str_today))
bst_rh2m = train_bst(X_tr, y_tr[:, 1], X_val, y_val[:, 1], cat_var_list)
bst_rh2m.save_model('bst_rh2m_gbdt_{}_new2.txt'.format(str_today))
bst_w10m = train_bst(X_tr, y_tr[:, 2], X_val, y_val[:, 2], cat_var_list)
bst_w10m.save_model('bst_w10m_gbdt_{}_new2.txt'.format(str_today))
...
df_submit.to_csv(f'forecast-2018{str_today}03_gbdt_new2.csv', index=False)
...
bst_t2m = train_bst(X_tr, y_tr[:, 0], X_val, y_val[:, 0], cat_var_list)
bst_t2m.save_model('bst_t2m_dart_{}_new2.txt'.format(str_today))
bst_rh2m = train_bst(X_tr, y_tr[:, 1], X_val, y_val[:, 1], cat_var_list)
bst_rh2m.save_model('bst_rh2m_dart_{}_new2.txt'.format(str_today))
bst_w10m = train_bst(X_tr, y_tr[:, 2], X_val, y_val[:, 2], cat_var_list)
bst_w10m.save_model('bst_w10m_dart_{}_new2.txt'.format(str_today))
df_submit = pd.DataFrame([
    [f'{item[0]}_{int(item[1]):02d}' for item in zip(cat_var_lblenc_dict['stations'].inverse_transform(df_test.stations), 
                                                     cat_var_lblenc_dict['foretimes'].inverse_transform(df_test.foretimes))], 
    (df_test.t2m_M + bst_t2m.predict(df_test_X)).tolist(), 
...
df_submit.to_csv(f'forecast-2018{str_today}03_dart_new2.csv', index=False)

df_submit1 = pd.read_csv(f'forecast-2018{str_today}03_gbdt_new2.csv')
df_submit2 = pd.read_csv(f'forecast-2018{str_today}03_dart_new2.csv')
...
df_submit1.to_csv(f'forecast-2018{str_today}03_new2.csv', index=False)